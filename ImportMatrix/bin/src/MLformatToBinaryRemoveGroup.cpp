...
int main(int argc, char* argv[]){
    ...
    *********************************************/
    ...
    boost::split(contentname, line, boost::is_any_of("\t"));
    contentname[0]="name";
    ...
    //check for group to delete
    uint64_t nSampledeleted=0;
    if (groupId2Delete!=""){
        for (uint64_t i = 0; i < content.size(); i++) {
            if (content[i]==groupId2Delete){
                maskGroups2delete.push_back(true);
                nSampledeleted++;
            }else{
                maskGroups2delete.push_back(false);
            }
        }
        
    }
    NumberOfColumnsIncludingLabel-=nSampledeleted;
    cout<<endl<<nSampledeleted<<"Samples deleted"<<endl;
    /*********************************************
    ...
    *********************************************/
    ...
    for (uint64_t i = 0; i < contentname.size(); i++) {
        if (maskGroups2delete.empty()|(maskGroups2delete[i]==false)){
            char bufhead[64];
            strcpy(bufhead, contentname[i].c_str());
